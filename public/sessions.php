...
// Démarrage de la session et récupération des informations de l'utilisateur connecté
...
// Récupération du nom de l'utilisateur pour un affichage personnalisé
$userName = Session::getCurrentUserName($conn, $userId);

// Traitement des actions utilisateur : création, mise à jour ou suppression de sessions
Session::handleActions($conn, $userId);

// Récupération de la liste des sessions associées à l'utilisateur
...
<body class="bg-gray-50 min-h-screen flex flex-col">
    ...
    <header class="bg-white shadow">
        <div class="container mx-auto px-4 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center space-x-2 mb-4 sm:mb-0">
                <!-- Icône calendrier pour contextualiser l'affichage -->
            ...
            <!-- Formulaire d'ajout  d'une nouvelle session -->
    ...
    <main class="container mx-auto flex-1 px-4 py-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach ($sessions as $s): ?>
                    <div class="bg-white rounded-lg shadow hover:shadow-lg transition flex flex-col">
                        ...
                        <!-- En-tête de la carte session : nom et icône suppression -->
                        <div class="bg-blue-600 text-white px-4 py-3 rounded-t-lg flex items-center relative">
                            ...
                            <a href="sessions.php?delete=<?= $s->id ?>" onclick="return confirm('Supprimer cette session ?')"
                                class="absolute right-4 text-red-200 hover:text-red-400" title="Supprimer">
                                🗑️
                            </a>
                        </div>

                        <!-- Corps de la carte : lien vers détails de la session -->
        ...
        <?php else: ?>
            <!-- Message affiché si l'utilisateur n'a aucune session -->
